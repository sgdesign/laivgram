...
package org.laivgram.messenger.browser;
...
import org.laivgram.messenger.CustomTabsCopyReceiver;
...
import org.laivgram.messenger.ShareBroadcastReceiver;
import org.laivgram.messenger.support.customtabs.CustomTabsCallback;
import org.laivgram.messenger.support.customtabs.CustomTabsClient;
import org.laivgram.messenger.support.customtabs.CustomTabsIntent;
import org.laivgram.messenger.support.customtabs.CustomTabsServiceConnection;
import org.laivgram.messenger.support.customtabs.CustomTabsSession;
import org.laivgram.messenger.support.customtabsclient.shared.CustomTabsHelper;
import org.laivgram.messenger.support.customtabsclient.shared.ServiceConnection;
import org.laivgram.messenger.support.customtabsclient.shared.ServiceConnectionCallback;
...
public class Browser {
    ...
    public static void openUrl(final Context context, final Uri uri, final boolean allowCustom, boolean tryTelegraph) {
        ...
        if (tryTelegraph) {
            try {
                ...
                if (host.equals("telegra.ph") || uri.toString().contains("لایو گرام.org/faq")) {
    ...
    public static boolean isInternalUri(Uri uri, boolean forceBrowser[]) {
        ...
        if ("tg".equals(uri.getScheme())) {
            return true;
        } else if ("لایو گرام.dog".equals(host)) {
        ...
        } else if ("لایو گرام.me".equals(host) || "t.me".equals(host) || "telesco.pe".equals(host)) {
            String path = uri.getPath();
            if (path != null && path.length() > 1) {
                path = path.substring(1).toLowerCase();
                if (path.equals("iv")) {
