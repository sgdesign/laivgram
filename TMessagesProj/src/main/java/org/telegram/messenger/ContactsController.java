...
public class ContactsController {
    ...
    public String getInviteText(int contacts) {
        String link = inviteLink == null ? "https://لایو گرام.org/dl" : inviteLink;
        if (contacts <= 1) {
    ...
    public void checkAppAccount() {
        ...
        Account[] accounts;
        try {
            accounts = am.getAccountsByType("org.laivgram.account");
        ...
        accounts = am.getAccountsByType("org.laivgram.messenger");
        ...
        if (recreateAccount) {
            ...
            if (UserConfig.isClientActivated()) {
                try {
                    currentAccount = new Account("" + UserConfig.getClientUserId(), "org.laivgram.messenger");
    ...
    public void deleteAllAppAccounts() {
        try {
            AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
            Account[] accounts = am.getAccountsByType("org.laivgram.messenger");
            for (int a = 0; a < accounts.length; a++) {
                am.removeAccount(accounts[a], null, null);
            }
        } catch (Exception e) {
        ...
        Utilities.globalQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                /*if (schedule) {
                    try {
                        AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
                        Account[] accounts = am.getAccountsByType("org.laivgram.account");
                        boolean recreateAccount = false;
                        if (UserConfig.isClientActivated()) {
                            if (accounts.length != 1) {
                                FileLog.e("detected account deletion!");
                                currentAccount = new Account(UserConfig.getCurrentUser().phone, "org.laivgram.account");
                                am.addAccountExplicitly(currentAccount, "", null);
                                AndroidUtilities.runOnUIThread(new Runnable() {
                                    @Override
                                    public void run() {
...
//        query.add(builder.build());
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.org.laivgram.messenger.android.profile");
        builder.withValue(ContactsContract.Data.DATA1, user.id);
        builder.withValue(ContactsContract.Data.DATA2, "لایو گرام Profile");
