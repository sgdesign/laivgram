package org.laivgram.messenger.voip;
...
import org.laivgram.messenger.StatsController;
...
import org.laivgram.ui.VoIPActivity;
import org.laivgram.ui.VoIPPermissionActivity;
...
public abstract class VoIPBaseService extends Service implements SensorEventListener, AudioManager.OnAudioFocusChangeListener, VoIPController.ConnectionStateListener{
	...
	public void onCreate() {
		...
		try {
			...
			cpuWakelock = ((PowerManager) getSystemService(POWER_SERVICE)).newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "لایو گرام-voip");
			cpuWakelock.acquire();
	...
	protected void configureDeviceForCall() {
		...
		try{
			if(proximity!=null){
				proximityWakelock=((PowerManager) getSystemService(Context.POWER_SERVICE)).newWakeLock(PROXIMITY_SCREEN_OFF_WAKE_LOCK, "لایو گرام-voip-prx");
